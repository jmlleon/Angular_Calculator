import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { environment } from 'src/environments/environment';
import { AuthObject } from '../models/AuthObject.model';
import { UserProfile } from '../models/UserProfile.model';
import { AuthService } from './auth.service';

@Injectable({
  providedIn: 'root'
})
export class ProfileService {

  AppUrl: string = '/PerfilUsuario';

  httpHeaders!:HttpHeaders;
  authObject!:AuthObject | null;

  constructor(private client:HttpClient, private authSvc:AuthService ) {

    this.authSvc.usuarioActual$.subscribe({
      next:(user)=>{ 
         this.authObject=user;
         this.httpHeaders = new HttpHeaders().set("Authorization", "Bearer " + this.authObject!==null ? this.authObject!.token.accessToken:"");
      
      }    
    })
   
   }

  GetProfileList(){

    return this.client.get<UserProfile[]>(`${environment.ApiUrl}${this.AppUrl}`, {headers:this.httpHeaders} )

  }

}
